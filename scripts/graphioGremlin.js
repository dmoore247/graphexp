...
var graphioGremlin = (function () {
	...
	function lineage_query(d) {
		var id = d.id;
		if (isNaN(id)) { // Add quotes if id is a string (not a number).
			id = '"' + id + '"';
		}
		//g.V(id).emit().repeat(out()).times(5).dedup().order()
		var gremlin_query_nodes = 'nodes = g.V(' + id + ').as("node").emit().repeat(both()).times(15).dedup()';
		var gremlin_query_edges = 'edges = g.V(' + id + ').emit().repeat(both()).times(15).bothE().dedup()';

		sendQuery(gremlin_query_nodes, gremlin_query_edges);
	}

	function ancestors_query(d) {
		var id = d.id;
		if (isNaN(id)) { // Add quotes if id is a string (not a number).
			id = '"' + id + '"';
		}
		//g.V(id).emit().repeat(out()).times(5).dedup().order()
		var gremlin_query_nodes = 'nodes = g.V(' + id + ').as("node").emit().repeat(__.in()).times(15).dedup()';
		var gremlin_query_edges = 'edges = g.V(' + id + ').emit().repeat(__.in()).times(15).bothE().dedup()';

		sendQuery(gremlin_query_nodes, gremlin_query_edges);
	}

	function decendents_query(d) {
		var id = d.id;
		if (isNaN(id)) { // Add quotes if id is a string (not a number).
			id = '"' + id + '"';
		}
		//g.V(id).emit().repeat(out()).times(5).dedup().order()
		var gremlin_query_nodes = 'nodes = g.V(' + id + ').as("node").emit().repeat(out()).times(15).dedup()';
		var gremlin_query_edges = 'edges = g.V(' + id + ').emit().repeat(out()).times(15).bothE().dedup()';

		sendQuery(gremlin_query_nodes, gremlin_query_edges, id);
	}

	function sendQuery(gremlin_query_nodes, gremlin_query_edges, id) {
		// while busy, show we're doing something in the messageArea.
		$('#messageArea').html('<h3>(loading)</h3>');
		var message = "<p>Query ID: " + id + "</p>"
		if (SINGLE_COMMANDS_AND_NO_VARS) {
			var nodeQuery = create_single_command(gremlin_query_nodes);
			var edgeQuery = create_single_command(gremlin_query_edges);
			send_to_server(nodeQuery, null, null, null, function (nodeData) {
				send_to_server(edgeQuery, null, null, null, function (edgeData) {
					var combinedData = [nodeData, edgeData];
					handle_server_answer(combinedData, 'click', id, message);
				});
			});
		} else {
			var gremlin_query = gremlin_query_nodes + '\n' + gremlin_query_edges + '\n' + '[nodes.toList(),edges.toList()]'
			send_to_server(gremlin_query, 'click', id, message);
		}
	}

	function neighbors_query(d) {
		console.log(d);
	...
	return {
		...
		neighbors_query: neighbors_query,
		ancestors_query: ancestors_query,
		decendents_query: decendents_query,
